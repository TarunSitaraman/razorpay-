...
| 8 | 🔒 **COMPLIANT ESCALATION** | A decision row: `message/sms` funded, with `✕ silent_retry — blocked by RBI_AFA_LIMIT: amount ₹23,354.51 exceeds the AFA-free limit ₹15,000.00` | "The agent wanted the cheaper action. RBI said no — above ₹15,000 a debit needs authentication. So it fell back to a payment link. Nobody wrote that rule into the recovery logic; the policy engine decided it." |
...
- **Yukti and propensity spend the same contact budget** (88 vs 89). Yukti is not
  winning by abstaining.
- **Only Yukti's interval excludes zero** (+103,958 [+38,351, +174,141] per 1k).
...
> Yukti's true effect is ₹315.02 per case against a per-case spread of
